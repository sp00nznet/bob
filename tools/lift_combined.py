...
def write_exports(exports):
    """Emit src/_exports.c: every lifted module's export table, so
    GetProcAddress can hand back a real far pointer instead of 0.

    A Win16 app that loads a component with LoadLibrary + GetProcAddress reads
    a 0 as "the file is missing" -- which is exactly what Bob's VBX loader
    reported. The (segment, offset) here is the GLOBAL one, so dispatch_far
    lands on the lifted function."""
    out = os.path.join(SRC, '_exports.c')
    rows = [e for e in exports if e[0]]
    with open(out, 'w', encoding='utf-8', newline=NL) as f:
        w = lambda s: f.write(s + NL)
        w('/* _exports.c - generated by lift_combined.py. */')
        w('#include "ne_exports.h"')
        w('')
        w('const NEExport g_ne_exports[] = {')
        for hinst, name, ordinal, seg, off in rows:
            w('    {0x%04X, "%s", %d, %d, 0x%04X},' % (hinst, name, ordinal, seg, off))
        w('};')
        w('const int g_ne_nexports =')
        w('    (int)(sizeof g_ne_exports / sizeof g_ne_exports[0]);')
    print(f"Wrote {out}: {len(rows)} exports")



def main():
    # Every lifted module's export table -> xmod, so a cross-module import
    # becomes a direct call into that module's lifted C instead of a Win16
    # stub. Segment numbers are global (each module's MODULES offset).
    #
    # Keyed by BOTH ordinal and exported name, because the two import forms
    # carry different things: an import-by-ordinal fixup carries the ordinal,
    # an import-by-NAME fixup carries a byte offset into the importing
    # module's imported names table and no ordinal at all. UTOPIA imports
    # UEXTRA entirely by name, so with only ordinal keys every blitter call
    # -- and the B$PEND..HOLE template the Access Basic runtime is built
    # from -- resolved to a no-op stub. See NELifter._xres.
    engine = parse_ne(MODULES[0][0])
    xmod = {}
    exports = []
    for path, offset, _ in MODULES:
        mod = os.path.basename(path).rsplit('.', 1)[0].upper()
        m_ne = engine if offset == 0 else parse_ne(path)
        m = {}
        for e in m_ne.entries:
            m[e.ordinal] = (e.segment + offset, e.offset)
            if e.name:
                m[e.name.upper()] = (e.segment + offset, e.offset)
        xmod[mod] = m
        # hinst is the module's DGROUP selector in the combined image --
        # the same handle ne_resources.c hands back from LoadLibrary.
        hinst = m_ne.auto_data_seg + offset if m_ne.auto_data_seg else 0
        for e in m_ne.entries:
            exports.append((hinst, e.name or '', e.ordinal,
                            e.segment + offset, e.offset))
        print(f"{mod} export entries: {len(m_ne.entries)} (hinst {hinst})")
